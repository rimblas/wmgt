$.widget('uc.nestedReport', {
  defaultTemplateMarker: '#DEFAULT_TEMPLATE#',
  classes: {
    nestedReportTrContainer       : 'uc_plugin_row',
    ...
    tdExpanded                    : 'uc--expanded',
    ajaxIndicatorRight            : 'uc--ajaxIndicator floatRight',
    ajaxIndicatorContent          : 'uc--ajaxIndicator content',
    ajaxIndicatorIcon             : 'fa fa-spin  fa-refresh',
    tableStrechReport             : 'uc--strechReport'
  },
...
//
  _getExpandedFrom: function( pElement ) {
    ...
    expanded = pElement.find('*').filter( function(pIndex, pElement){
      ...
      if ( self.data('uc-nestedReport') != undefined && self.nestedReport('isExpanded') == true ) {
        return true;
      }
    } );
...
//
  throwError: function( pError ){
    ...
    setTimeout( $.proxy(function(){
      ...
      this.expanded = true;
      this.td.data('uc-nestedReport-owner', this.element);
...
//
  expand: function( pForceDuration ){
    ...
    //before slide down
    apex.event.trigger(this.affectedElement, 'uc_default_callback', this.getEventData());

    //uc-nestedReport-owner is used to find out triggering element
...
//
  defaultCallbackEvent_expandAll: function( pEvent ){
    ...
    toBeExpanded.each( $.proxy( function( pIdx, pElem ){
      ...
      if ( self.data('uc-nestedReport') == undefined ) {
        self.trigger('click');
      }
  ...
  _showSpinner: function(){
    ...
    if ( this.settings.isSpinnerTdIcon ) {
      ...
      if ( this.td.children().length > 0 ) {
        this.td.data('uc-nestedReport-content', this.td.children().detach() );
      }
      else {
        this.td.data('uc-nestedReport-content', this.td.text() );
      }
    ...
    if ( this.settings.isLoadingIndicator ) {
      this.spinner.fadeOut(400, $.proxy(function(){ 
        ...
        if ( this.settings.isSpinnerTdContent ) {
          this.td.html( this.td.data('uc-nestedReport-content') )
        }
      }, this));
    }
  },
...
//
  _getErrorTemplate: function( pError ){
    ...
    div = $(''+
      ...
      '    <span class="uc--errorTitle">'+pError.title+'</span>   '+
      '  </div>                                                        '+
      '  <div>                                                         '+
      '    <span class="uc--errorAddInfo">'+pError.text+'</span>  '+
      '  </div>                                                        '+
      '  <div class="uc--techError">                              '+
      '    <pre>'+pError.pre+'</pre>                                   '+
      '  </div>                                                        '+
      '</div>                                                          '+
    '');
...
//
  _expandedInReport: function(){
    ...
    this.table.find('td').each( function(){
      var 
        self = $(this),
        tdOwner = self.data('uc-nestedReport-owner');
...
//
  _highlight: function( pEvent ){
    ...
    if ( this.expanded ) {
      ...
      this.row.addClass('uc--hover');
      this.nestedReport.tr.addClass('uc--hover');
    }
  },
//
//
